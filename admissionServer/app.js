...
var getDbKey = function (req, callback) {
  return callback(null, global.COMMON_CONFS.getigkv_Admission());
};
...
// --- Corrected Structure ---
// 1. Path to your RECRUITMENT UPLOADS folder
const admissionUploadsPath = path.join(__dirname, "../CET");
// 2. Path to your general PUBLIC ASSETS folder
const globalPublicPath = path.join(__dirname, "../../../public");
...
app.use(
  ...
  express.static(admissionUploadsPath)
);
// B) Make files from the 'public' folder available (Best Practice: use a virtual path)
app.use("/public", express.static(globalPublicPath));

var initAllFiles = function () {
  global.apiPrefix = "/admissionApi"; // this is used to set the prefix for all the routes.
