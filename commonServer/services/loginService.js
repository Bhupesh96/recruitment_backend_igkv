...
var login = {
          ...
          request.session.save((err) => {
            if (err) {
              ...
              Object.keys(designation_office_details).forEach((key) => {
                designation_office_details[key] = Array.from(
                  designation_office_details[key]
                );
              });
              request.session.emp_id = user["emp_id"];
              request.session.user_id = user["user_id"];
              request.session.district_id = user["district_id"];
              request.session.designation_arr = user["designation_ids"]
                ?.split(",")
                .map(Number);
              request.session.designation_office_details =
                designation_office_details;
              // console.log(designation_office_details,officeDetails);

              login.updateSessionTable(
                dbkey,
                request,
                request.session.id,
                user["user_id"],
                function (err, res) {
                  let data = {
                    ...
                    today: user["today"],
                    designation_arr: user["designation_ids"]
                      ?.split(",")
                      .map(Number),
                    officeDetails: officeDetails,
                  };
                  let cookieString = CryptoJS.AES.encrypt(
                    JSON.stringify(data),
                    "UFP_secret_key"
                  ).toString();
  ...
  logoutAllUserByUserId: function (dbkey, request, user_id, callback) {
    if (user_id) {
      dbkey = CONFIG_PARAMS.getloginDBDetails();
      var queryObj =
        SECURITY_SERVICE_QUERIES.getdeleteUserAllSessionquery(user_id);
      DB_SERVICE.executeQueryWithParameters(
        dbkey,
        queryObj.query,
        queryObj.params,
  ...
  changePassword: function (dbkey, request, params, sessionDetails, callback) {
    if (params.user_id && params.password) {
      ...
      let pass = CryptoJS.AES.decrypt(
        params.password,
        "08t16e502526fesanfjh8nasd2"
      ); //
      let dPass = pass.toString(CryptoJS.enc.Utf8);
      let hash_password = "";
      // hash the new password
      ENCRYPTION_SERVICE.encrypt(dPass)
        .then((data) => {
          hash_password = data;
          let updateObj = {
            password: hash_password,
            password_flag: 1,
            password_update_dtstamp: new Date(),
          };
          let whereObj = { user_id: params.user_id };
          let queryObj = DB_SERVICE.getUpdateQueryAndparams(
            updateObj,
            whereObj,
            "users"
          );
  ...
  changePasswordWithCheck: function (
    dbkey,
    request,
    params,
    sessionDetails,
    ...
    if (params.user_id && params.password && params.current_password) {
      ...
      return callback(
        "user id ,current_password and password not sent in param"
      );
    }
  },
  ...
  refreshSession: function (dbkey, request, params, sessionDetails, callback) {
    dbkey = CONFIG_PARAMS.getloginDBDetails();
    var queryObj = SECURITY_SERVICE_QUERIES.getLoginDetailsQuery(
  ...
  resetPassword: function (dbkey, request, params, sessionDetails, callback) {
    ...
    let div_id = +params["div_id"],
      ...
      tehsil_id = params["tehsil_id"],
      subdistrict_code = +params["subdistrict_code"];
        ...
        function (cback0) {
          if (typeOfCase && arrOfCase.includes(typeOfCase)) {
            if (typeOfCase == 1 && user_type && typeof user_type == "number") {
              ...
              typeof user_type == "number" &&
              typeof district_id == "number" &&
              typeof subdistrict_code == "number"
            ) {
              return cback0();
            } else {
              return cback0({
                code: `ERROR_REQUIRED_FIELDS`,
                message: `Sufficient Data Not Provided`,
              });
            }
          } else {
            return cback0({
              message: `INVALID Value For Case that is ${typeOfCase}`,
              code: `INVALID_CASE`,
            });
          }
        },
        function (cback2) {
          qAndP = LOGIN_SERVICE_QUERIES.getPasswordResetQueryParam(
            typeOfCase,
            id,
            user_type,
            div_id,
            ...
            function (e, r) {
              if (e) {
                ...
                return cback2({
                  ...
                  message: `Multiple OR Zero Password Reseted`,
                });
              }
            }
          );
